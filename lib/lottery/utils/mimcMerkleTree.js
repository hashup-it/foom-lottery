...
import { leBufferToBigint, hexToBigint, bigintToHex } from "./bigint.js";
...
async function getLines(path) {
  try {
    ...
    const gzRes = await fetch(`/www/${path}.gz`);
    if (gzRes.ok) {
...
async function getIndexRand(hashstr,betIndex) {
...
async function getWaitingList(nextIndex,hashesLength){
  const lines = await getLines("www/waiting.csv");
