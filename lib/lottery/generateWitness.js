...
import { hexToBigint, bigintToHex, leBigintToBuffer, reverseBits } from './utils/bigint.js'
import { pedersenHash } from './utils/pedersen.js'
import { mimcsponge2, mimcsponge3 } from './utils/mimcsponge.js'
...
export async function generateWitness(inputs) {
  ...
  console.log('have inputs:', inputs)
  ...
  const terces = reverseBits(
    (secret + rand) % 21888242871839275222246405745257275088548364400416034343698204186575808495617n,
    31 * 8
  )
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  generateWitness(process.argv.slice(2))
    .then(({ witness }) => {
